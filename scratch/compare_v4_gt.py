...
with open(r'c:\Users\Tomas\Documents\LAB3\CLAUDIO_AI\out_jijiji_v4\detecciones.json', 'r') as f:
...
for p_i, p in enumerate(pred_data):
    ...
    if min_dist <= threshold_cad:
        ...
        pred_to_gt[p_i] = (best_gt_i, min_dist)
...
if true_positives > 0:
    print("\n--- MATCHED DETECTIONS CONFIDENCE ---")
    confs = [pred_data[p_i]['conf'] for p_i in matched_pred]
    ...
    print("\nIndividual matches:")
    for p_i in sorted(matched_pred, key=lambda i: -pred_data[i]['conf']):
        p = pred_data[p_i]
        gt_i, dist = pred_to_gt[p_i]
        g = gt_data[gt_i]
        print(f"  Prediction #{p_i+1:2d} -> GT #{gt_i+1:2d} | Conf: {p['conf']:.4f} | CAD: ({p['x_cad']:.2f}, {p['y_cad']:.2f}) | Dist: {dist:.3f}")
